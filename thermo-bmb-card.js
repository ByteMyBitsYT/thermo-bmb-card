...
setConfig(config) {
  ...
  this.config.temperaturefontsize = parseFloat(config.temperaturefontsize) || 0;
  this.config.humidityfontsize = parseFloat(config.humidityfontsize) || 0;
  this.config.labelfontsize = parseFloat(config.labelfontsize) || 0;
  this.config.temperatureicon = config.temperatureicon || 'mdi:thermometer';
  ...
  // Set Temperature range for colors
  this.config.maxTemp = config.maxTemp || 120;
  this.config.midTemp = config.midTemp || 60;
  this.config.minTemp = config.minTemp || 40;
  // Set Temperature Gradient Colors
  this.config.maxColor = config.maxColor || '#ff0000';
  this.config.midColor = config.midColor || '#00ff00';
  this.config.minColor = config.minColor || '#0000ff';
...
_render() {
  ...
  .temperature {
    ...
    width: 100%;
    text-align: center;
    margin-top: var(--dynamic-temp-container);
    margin-bottom: var(--dynamic-temp-container);
  ...
  .label {
    ...
    width: 100%;
    text-align: center;
    margin-top: var(--dynamic-div-margin-container);
    line-height: 100%;      
    z-index: 1;
  }
  hr {
  ...
  .humidity {
    ...
    width: 100%;
    text-align: center;
    margin-top: var(--dynamic-hum-container);
...
`;
  ...
  if (haCard) {
    haCard.addEventListener('click', (event) => {
      let targetClass = event.target.className;
      if (targetClass === 'temperature') {
        this.handlePopup(this.config.temperature_sensor);
      } else if (targetClass === 'humidity') {
        this.handlePopup(this.config.humidity_sensor);
      }
    });
  }
  ...
  if (humidityElementInRender) {
    humidityElementInRender.addEventListener('click', (event) => {
      this.handlePopup(this.config.humidity_sensor);
  ...
  if (tempElementInRender) {
    tempElementInRender.addEventListener('click', (event) => {
      this.handlePopup(this.config.temperature_sensor);
      event.stopPropagation();
    });
  }
...
let temperatureFontSizeFactor = 0.24 + (this.config.temperaturefontsize * 0.01);
let humidityFontSizeFactor = 0.15 + (this.config.humidityfontsize * 0.01);
let labelFontSizeFactor = 0.06 + (this.config.labelfontsize * 0.01);
let tempcontainerSizeFactor = 0.015 + (this.config.temperaturefontsize * 0.008);
let humcontainerSizeFactor = -0.02 - (this.config.humidityfontsize * 0.001);
let labeldivSizeFactor = 0.06 + (this.config.labelfontsize * 0.001);
let labeldivMarginFactor = 0.015 + (this.config.labelfontsize * 0.001);
...
const elements = [
  ...
  {selector: '.sensorsdata', variable: '--label-div-height', factor: labeldivSizeFactor},
  {selector: '.temperature', variable: '--temperature-font-size', factor: temperatureFontSizeFactor},
  {selector: '.humidity', variable: '--humidity-font-size', factor: humidityFontSizeFactor},
  {selector: '.label', variable: '--label-font-size', factor: labelFontSizeFactor},
  ...
  {selector: '.sensorsdata', variable: '--dynamic-div-margin-container', factor: labeldivMarginFactor},
  {selector: '.sensorsdata', variable: '--dynamic-temp-container', factor: tempcontainerSizeFactor}, 
  {selector: '.sensorsdata', variable: '--dynamic-hum-container', factor: humcontainerSizeFactor},    
...
function generateGradientColors(maxColor, midColor, minColor, maxTemp, midTemp, minTemp) {
  const colorMap = new Map();
  colorMap.set(maxTemp, maxColor);
  colorMap.set(midTemp, midColor);
  colorMap.set(minTemp, minColor);
  return function getTemperatureColor(temperature) {
    let lowKey = minTemp;
    let highKey = maxTemp;
    for (const [key, value] of colorMap) {
      if (temperature >= key && key >= lowKey) {
        lowKey = key;
      }
      if (temperature <= key && key <= highKey) {
        highKey = key;
      }
    }
    const lowColor = colorMap.get(lowKey);
    const highColor = colorMap.get(highKey);
    if (lowColor === highColor) {
      return lowColor;
    }
    const ratio = (temperature - lowKey) / (highKey - lowKey);
    const newColor = interpolateColors(lowColor, highColor, ratio);
    return newColor;
  };
}
...
const getTemperatureColor = generateGradientColors('#ff0000', '#00ff00', '#0000ff', 120, 60, 40);
...
if (temperatureSensor) {
  ...
  const tempeIcon = this.config.temperatureicon || 'mdi:thermometer';
  // Check if temperatureElement exists
  if (temperatureElement) {
    temperatureElement.innerHTML = `<ha-icon icon="${tempeIcon}" class="temperature-icon"></ha-icon>${temperatureValue}<span style="font-size: var(--temperature-symbol-size);">°F</span>`;
    const TempIcon = this.shadowRoot.querySelector('.temperature-icon');
    const getTemperatureColor = generateGradientColors(this.config.maxColor, this.config.midColor, this.config.minColor, this.config.maxTemp, this.config.midTemp, this.config.minTemp);
    if (TempIcon) {
...
// Define custom icon paths for spinners
const THR_ICONS_MAP = {
  "therm-mid-pattern": {
    "keywords": [],
...
// Inject Custom Icons into Home Assistant
