...
// v0.6(ish)
  ...
  setConfig(config) {
    ...
    if (!config.temperature_sensor || !config.humidity_sensor || !config.label) {
  ...
  _render() {
    ...
    .outer {
      ...
      top: 0;
      left: 0;
      animation: rotate-center 200s linear 0s infinite normal none;
    ...
    .midoutrotate {
      ...
      animation: rotate-center 150s linear 0s infinite reverse none;
    ...
    .midpattern {
      ...
      left: 0;
      color: #000000;
      animation: rotate-center 250s linear 0s infinite normal none;
    ...
    .innerlines {
      ...
      top: 0;
      left: 0;
      animation: rotate-center 500s linear 0s infinite normal none;
...
// Dynamic Control for custom --vARRGS
  ...
  const elements = [
    ...
    {selector: '.temperature', variable: '--temperature-font-size', factor: 0.20},
    ...
    {selector: '.label', variable: '--label-font-size', factor: 0.05},
    {selector: '.sensorsdata', variable: '--humidity-div-height', factor: 0.09},
    {selector: '.humidity', variable: '--humidity-font-size', factor: 0.1},
    ...
    {selector: '.sensorsdata', variable: '--dynamic-margin-container', factor: 0.012},
  ...
  if (temperatureSensor) {
    ...
    if (temperatureElement) {
      temperatureElement.innerHTML = `<ha-icon icon="mdi:thermometer" class="temperature-icon"></ha-icon>${temperatureValue}<span style="font-size: var(--temperature-symbol-size);">°F</span>`;
        ...
        const midRotateIcon = this.shadowRoot.querySelector('.midrotate');
        if (midRotateIcon) {
          midRotateIcon.style.setProperty('--midrotate-icon-color', getTemperatureColor(temperatureValue));

          let criticalValue = this.config.critical || 80;
          if (temperatureValue > criticalValue) {
            midRotateIcon.style.animation = 'rotate-center 60s linear 0s infinite reverse, blink 2s linear 0s infinite normal';
          } else {
            midRotateIcon.style.animation = 'rotate-center 60s linear 0s infinite reverse';
          }
